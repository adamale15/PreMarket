...
function calculateProbability(tweet: TwitterTweet): number {
  ...
  if (metrics.retweet_count > 100) base += 5;
...
function tweetToTrend(
  ...
  const title = tweetText.length > 100 
    ? `${tweetText.substring(0, 97)}...` 
    : tweetText;
  
  // Create summary with engagement metrics
  const metrics = tweet.public_metrics || {};
  const engagementText = metrics.retweet_count > 0 || metrics.like_count > 0
    ? ` (${metrics.retweet_count} retweets, ${metrics.like_count} likes)`
    : "";
...
export async function GET(request: Request) {
  ...
  if (!TWITTER_BEARER_TOKEN) {
    ...
    return NextResponse.json({
      trends: [],
      message: "Twitter Bearer Token not configured. Please add TWITTER_BEARER_TOKEN to your .env file.",
    });
  }
  ...
  try {
    ...
    if (interests.length > 0) {
      ...
      console.log(`Twitter query for interests [${interests.join(", ")}]: ${twitterQuery}`);
    } else {
    ...
    if (!response.ok) {
      if (response.status === 429) {
        ...
        let errorMessage = "Twitter API rate limit exceeded. Please try again later.";
        if (resetTime) {
        ...
        return NextResponse.json({
          ...
          status: 429,
          headers: {
            "Retry-After": resetTime ? String(Math.ceil((parseInt(resetTime) * 1000 - Date.now()) / 1000)) : "900",
          },
        });
      }
    ...
    const trends = data.data
      ...
      .map((tweet, index) => {
        const author = tweet.author_id ? authorMap.get(tweet.author_id) || null : null;
